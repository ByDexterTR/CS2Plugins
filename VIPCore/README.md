...
Modüler VIP sistemi. 40'tan fazla yerleşik VIP özelliği (modül), grup tabanlı yetkilendirme, JSON veya MySQL depolama ve üç farklı menü tipiyle eksiksiz bir VIP altyapısı sunar.

## Özellikler

- **40+ yerleşik modül** — hepsi tek DLL içinde, otomatik keşfedilir (reflection)
...
- [CounterStrikeSharp](https://github.com/roflmuffin/CounterStrikeSharp) v1.0.370+
...
| `AntiFlash` | Flash etkisini azaltır/engeller | `{ "self": true, "enemy": true, "teammates": true }` |
...
| `AutoHS` | Vücut vuruşlarını çarpanla güçlendirir | `{ "multiplier": 4, "only_with_weapon": "" }` |
...
| `BuyTeamWeapon` | Karşı takım silahlarını satın alma | `{ "ak47": true, "m4a4": true, ... }` |
...
| `DamageDealt` | Verilen hasarı artırır | `{ "percent": 50, "only_with_weapon": "" }` |
| `DamageResist` | Alınan hasarı azaltır | `{ "percent": 40, "only_with_weapon": "" }` |
| `DecoyTeleport` | Decoy'un düştüğü yere ışınlanma | `{ "limit": 3 }` |
| `DefuseKit` | Spawn'da imha kiti (CT) | `true` |
| `ExtraHP` | Spawn HP değeri | `150` |
| `ExtraJump` | Çift/çoklu zıplama | `{ "count": 2, "limit": 0 }` |
...
| `ReflectDamage` | Hasar yansıtma | `{ "reflect_percent": 50, "max_per_shot": 100, "only_with_weapon": "" }` |
...
| `Vampire` | Verilen hasar kadar can çalma | `{ "heal_percent": 75, "only_with_weapon": "", "max_overheal": 120 }` |
| `VIPChat` | VIP'lere özel sohbet kanalı | `true` |
| `Zeus` | Spawn'da taser | `true` |
