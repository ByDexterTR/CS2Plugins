...
| `CustomWeaponModel` | Custom look for a weapon; a number also changes the model in your hands, a file path only changes the one on the ground. The menu groups the entries by `weapon`, so every weapon keeps its own choice. Each knife can be given its own list (`weapon_knife_karambit`, `weapon_knife_kukri`, `weapon_bayonet` …); `weapon_knife` covers every knife that has no list of its own. A thrown grenade keeps the chosen model in the air as well; `throw: false` turns that off for a single entry, for models that vanish at range or fly off their own origin | `[{ "name": "M4A4 - AK47", "weapon": "weapon_m4a1", "model": "weapons/models/ak47/weapon_rif_ak47.vmdl", "throw": true }]` |
...
| `ExtraSpeed` | Speed multiplier. Speed bonuses stack: with `1.3` here and `+0.05` from `Adrenaline` the VIP moves at `1.35` | `{ "multiplier": 1.3, "only_with_weapon": "" }` |
...
| `HealthshotEffect` | Using a healthshot starts the effect picked from the menu for `time` seconds: `speed`, `strength` (extra damage), `heal`, `poison`, `slow`, `wallhack`, `radarhack`, `magnetic`, `resist` (takes `percent` less damage), `godmode` (takes no damage), `gravity` (gravity becomes `scale`), `infiniteammo` (the clip refills after every shot), `invisible` (hidden from enemies like `Invisibility`), `size` (the player becomes `scale` times their size and returns to the old size when it ends), `norecoil` (recoil left is `recoilpercent`, like `NoRecoil`). `radius: 0` means the effect only covers the player | `{ "speed": { "speed_multiplier": 1.3, "time": 5 }, "strength": { "damage_multiplier": 1.25, "time": 5, "radius": 0 }, "wallhack": { "time": 5, "radius": 0, "only_mode": 0 }, "resist": { "percent": 50, "time": 5 }, "godmode": { "time": 3, "limit": 1 }, "gravity": { "scale": 0.5, "time": 5 }, "infiniteammo": { "time": 5 }, "invisible": { "time": 3, "limit": 1 }, "size": { "scale": 0.7, "time": 5 }, "norecoil": { "recoilpercent": 0.0, "time": 5 } }` |
| `HitMarker` | Shows a particle next to your crosshair when you hit an enemy, only you and your spectators see it. `headshot` is an optional second particle for head hits, `duration` how long it stays, `distance` how far in front of the camera it sits (larger = bigger on screen; `0` for particles built as a screen effect, they are drawn on the screen itself and ignore the distance), `offset_x` / `offset_y` move it away from the center. It is glued to the crosshair every tick, so it stays in place while you turn | `[{ "name": "Impact", "particle": "particles/impact_fx/impact_hit_effect.vpcf", "headshot": "particles/impact_fx/impact_hit_effect_kill.vpcf", "duration": 0.25, "distance": 60 }]` |
...
| `NoRecoil` | Weapon recoil is reduced. `recoilpercent` is the recoil left (`0.0` – `1.0`): `0.0` none, `1.0` normal, `0.5` half. The spray pattern climbs slower and the aim punch is scaled down on every shot. `only_stance` and `only_with_weapon` work like in `RapidFire`. Only recoil changes; the spread while moving stays the same | `{ "only_with_weapon": "", "only_stance": 0, "recoilpercent": 0.0 }` |
...
| `Pet` | A companion that follows the player. It walks behind them, plays the run animation while moving and the idle animation while still; `flying` keeps it in the air at `height` with a gentle bob. Removed on death (can be hidden with `css_hidefx`). Animation names come from the model. `yaw` turns the model on its own axis, for models that were not built facing forward | `[{ "name": "Beaver", "model": "models/pets/beaver.vmdl", "flying": false, "speed": 260, "distance": 55, "height": 45, "scale": 1.0, "yaw": 0, "animations": { "idle": "@courier_idle", "run": "@courier_run", "death": "@courier_death", "spawn": "@courier_spawn" } }]` |
...
| `RapidFire` | `firepercent` is the fire rate (`0.1` – `2.0`): `1.0` normal, `2.0` fastest, below that is slower. `only_stance` picks when it works: `0` always, `1` while walking, `2` while crouching, `3` while jumping, `4` while standing still; the digits can be combined (`13` = walking or jumping). Recoil moved to `NoRecoil` | `{ "only_with_weapon": "", "only_stance": 0, "firepercent": 2.0 }` |
...
| `Silent` | Hides footsteps from other players; blinks with `duration_on`/`duration_off` (`duration_off: 0` = always silent) | `{ "only_with_weapon": "", "duration_on": 1, "duration_off": 0 }` |
...
- The same entries, except `C4Effect`, also accept a `team` field: `"T"` or `"CT"` shows and applies the entry only to that team, anything else or leaving it empty means both teams. `C4Effect` does not need it, because planting is always T and defusing is always CT.
