...
| `ExtraSpeed` | Hız çarpanı. Hız bonusları toplanır: burada `1.3` ve `Adrenaline`'dan `+0.05` varsa VIP `1.35` hızla koşar | `{ "multiplier": 1.3, "only_with_weapon": "" }` |
...
| `HealthshotEffect` | Healthshot kullanınca menüden seçilen etki `time` saniye başlar: `speed` hız, `strength` ekstra hasar, `heal`, `poison`, `slow`, `wallhack`, `radarhack`, `magnetic`, `resist` (`percent` kadar az hasar alır), `godmode` (hiç hasar almaz), `gravity` (yerçekimi `scale` olur), `infiniteammo` (her atıştan sonra şarjör dolar), `invisible` (`Invisibility` gibi rakiplerden gizlenir), `size` (oyuncu `scale` katı boyuta girer, bitince eski boyutuna döner), `norecoil` (`NoRecoil` gibi, kalan sekme `recoilpercent`). `radius: 0` etkinin sadece oyuncuya işlemesi demek | `{ "speed": { "speed_multiplier": 1.3, "time": 5 }, "strength": { "damage_multiplier": 1.25, "time": 5, "radius": 0 }, "wallhack": { "time": 5, "radius": 0, "only_mode": 0 }, "resist": { "percent": 50, "time": 5 }, "godmode": { "time": 3, "limit": 1 }, "gravity": { "scale": 0.5, "time": 5 }, "infiniteammo": { "time": 5 }, "invisible": { "time": 3, "limit": 1 }, "size": { "scale": 0.7, "time": 5 }, "norecoil": { "recoilpercent": 0.0, "time": 5 } }` |
| `HitMarker` | Düşmana vurunca nişangahının yanında partikül gösterir, sadece sen ve seni izleyenler görür. `headshot` kafa vuruşu için ayrı partikül, `duration` ekranda kalma süresi, `distance` kameranın kaç birim önünde duracağı (büyüdükçe ekranda büyür; ekran efekti olarak hazırlanmış partiküller için `0`, onlar doğrudan ekrana çizilir ve mesafeye bakmaz), `offset_x` / `offset_y` merkezden kaydırır. Her tick nişangaha sabitlenir, dönerken yerinde kalır | `[{ "name": "Impact", "particle": "particles/impact_fx/impact_hit_effect.vpcf", "headshot": "particles/impact_fx/impact_hit_effect_kill.vpcf", "duration": 0.25, "distance": 60 }]` |
...
| `NoRecoil` | Silah sekmesini azaltır. `recoilpercent` kalan sekme (`0.0` – `1.0`): `0.0` sekme yok, `1.0` normal, `0.5` yarı yarıya. Sprey deseni daha yavaş tırmanır ve her atışta nişangah tepmesi küçülür. `only_stance` ve `only_with_weapon` `RapidFire`'daki gibi çalışır. Yalnız sekme değişir, yürürken mermi dağılması aynı kalır | `{ "only_with_weapon": "", "only_stance": 0, "recoilpercent": 0.0 }` |
...
| `Pet` | Oyuncuyu takip eden yoldaş. Arkasından yürür, hareket ederken koşma, dururken bekleme animasyonunu oynatır; `flying` onu `height` yüksekliğinde havada tutar ve hafifçe salındırır. Ölünce silinir (`css_hidefx` ile gizlenebilir). Animasyon adları modelden gelir. `yaw` modeli kendi ekseninde çevirir, öne bakacak şekilde hazırlanmamış modeller için | `[{ "name": "Kunduz", "model": "models/pets/beaver.vmdl", "flying": false, "speed": 260, "distance": 55, "height": 45, "scale": 1.0, "yaw": 0, "animations": { "idle": "@courier_idle", "run": "@courier_run", "death": "@courier_death", "spawn": "@courier_spawn" } }]` |
...
| `RapidFire` | `firepercent` atış hızı (`0.1` – `2.0`): `1.0` normal, `2.0` en hızlı, altı yavaşlatır. `only_stance` ne zaman çalışacağını seçer: `0` her zaman, `1` yürürken, `2` eğilirken, `3` zıplarken, `4` sabit dururken; rakamlar birleştirilebilir (`13` = yürürken veya zıplarken). Sekme `NoRecoil` modülüne taşındı | `{ "only_with_weapon": "", "only_stance": 0, "firepercent": 2.0 }` |
...
- Aynı kayıtlar, `C4Effect` hariç, `team` alanını da alır: `"T"` veya `"CT"` kaydı yalnız o takıma gösterir ve uygular, başka bir değer ya da boş bırakmak iki takım demektir. `C4Effect`'te gerek yok, çünkü bomba kurmak hep T, imha etmek hep CT işidir.
