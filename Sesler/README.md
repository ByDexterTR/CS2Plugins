...
- **MySQL veya SQLite** desteği (biri başarısız olursa diğerine otomatik geçer); tablo otomatik oluşturulur
...
3. Varsayılan olarak SQLite kullanılır (eklenti klasöründe `Sesler.sqlite`); MySQL için config'i düzenleyin.
...
| --- | --- | --- | --- |
| `Database.provider` | string | `"sqlite"` | `"sqlite"` veya `"mysql"` |
